    ...
    def _load(self):
        ...
        if self.backend == "tensorrt":
            ...
            if not trtHandler.check_engine_exists(base_trt_engine_name):
                ...
                flownet_engine = trtHandler.build_engine(self.flownet, self.dtype, self.device, flownet_inputs, trt_engine_name=base_trt_engine_name, trt_multi_precision_engine=False, dynamic_shapes=flownet_dynamic_shapes,)
                trtHandler.save_engine(flownet_engine, base_trt_engine_name, flownet_inputs)
                TorchUtils.clear_cache()
                if self.encode:
                    encode_engine = trtHandler.build_engine(self.encode, self.dtype, self.device, encode_inputs, trt_engine_name=encode_trt_engine_name, trt_multi_precision_engine=False, dynamic_shapes=encode_dynamic_shapes,)
